...
export async function getCertificates({ take, skip, orderBy, searchQuery, filters = [] }: GetCertificatesProps) {
  ...
  const buildWhere = (excludeKey?: string): Prisma.ElectricalInstallationConditionReportWhereInput => {
    const filterConditions: Prisma.ElectricalInstallationConditionReportWhereInput = filters.reduce((acc, filter) => {
      ...
      if (filter.id === "startDate" && typeof filter.value === "object" && filter.value !== null) {
